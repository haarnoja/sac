...
class RemoteSampler(Sampler, Serializable):
    def __init__(self, **kwargs):
        ...
        self._final_distance_from_goal = 0
    ...
    def sample(self):
        ...
        if len(path_ready) or not self.batch_ready():
            ...
            if 'distance_from_goal' in infos[0]:
                self._final_distance_from_goal = infos[-1]['distance_from_goal']
            if 'avoid_cost' in infos[0]:
                self._avoid_cost = np.sum([info['avoid_cost'] for info in infos])
            if 'pos_cost' in infos[0]:
                self._pos_cost = np.sum([info['pos_cost'] for info in infos])
            if 'action_cost' in infos[0]:
                self._action_cost = np.sum([info['action_cost'] for info in infos])
            self._n_episodes += 1
    ...
    def log_diagnostics(self):
        ...
        logger.record_tabular('final_distance_from_goal', self._final_distance_from_goal)
        logger.record_tabular('pos_cost', self._pos_cost)
        logger.record_tabular('avoid_cost', self._avoid_cost)
        logger.record_tabular('action_cost', self._action_cost)
